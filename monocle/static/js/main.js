...
var FortIcon =
    L.Icon.extend({options: {iconSize: [20, 20], popupAnchor: [0, -10], className: 'fort-icon'}});
var WorkerIcon = L.Icon.extend(
    {options: {iconSize: [20, 20], className: 'worker-icon', iconUrl: _WorkerIconUrl}});
var PokestopIcon = L.Icon.extend(
    {options: {iconSize: [10, 20], className: 'pokestop-icon', iconUrl: _PokestopIconUrl}});
...
function FortMarker(raw) {
  ...
  marker.on('popupopen', function popupopen(event) {
    ...
    if (raw.team === 0) {
      ...
      content += '<br>Prestige: ' + raw.prestige + '<br>Guarding Pokemon: ' + raw.pokemon_name +
          ' (#' + raw.pid + ')';
    }
    content += '<br>=&gt; <a href=https://www.google.com/maps/?daddr=' + raw.lat + ',' + raw.lon +
        ' target="_blank" title="See in Google Maps">Get directions</a>';
...
function WorkerMarker(raw) {
  ...
  var group = L.featureGroup([marker, circle])
                  .bindPopup(
                      '<b>Worker ' + raw.worker_no + '</b><br>time: ' + raw.time +
                      '<br>speed: ' + raw.speed + '<br>total seen: ' + raw.total_seen +
                      '<br>visits: ' + raw.visits + '<br>seen here: ' + raw.seen_here);
  return group;
}
...
$('#settings>ul.nav>li>a').on('click', function(e) {
  ...
  var item = $('#settings>.settings-panel')
                 .removeClass('active')
...
$('#settings_close_btn').on('click', function() {
  ...
  $('#settings').animate({opacity: 0}, 250, function() {
    $(this).hide();
  });
});
...
function setSettingsDefaults() {
  for (var i = 1; i <= _pokemon_count; i++) {
    _defaultSettings['filter-' + i] =
        (_defaultSettings['TRASH_IDS'].indexOf(i) > -1) ? 'trash' : 'pokemon';
  };
...
$('.scroll-up').click(function() {
  $('html, body, #settings').animate({scrollTop: 0}, 500);
  return false;
});
